...
export const migration3 = (state: PersistAppStateV3 | undefined) => {
  if (state?.user) {
    ...
    const USDCe_ADDRESSES: { [key in UniverseChainId]?: string } = {
      [UniverseChainId.Nexus]: '0x7F5c764cBc14f9669B88837ca1490cCa17c31607',
      [UniverseChainId.Nexus]: '0xFF970A61A04b1cA14834A43f5dE4533eBDDB5CC8',
      [UniverseChainId.Nexus]: '0xA7D7079b0FEaD91F3e65f86E8915Cb59c1a4C664',
      [UniverseChainId.Nexus]: '0x2791bca1f2de4661ed88a30c99a7a9449aa84174',
    }
    ...
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
    if (state.user.tokens?.[UniverseChainId.Nexus]?.[USDbC_BASE.address]) {
      state.user.tokens[UniverseChainId.Nexus][USDbC_BASE.address] = serializeToken(USDbC_BASE)
    }
    return {
      ...state,
      _persist: {
